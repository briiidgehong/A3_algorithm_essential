...
24/01/19:
    소수판별 / gcd, lcm / 진수변환 정리 완료
    각 문제들 풀었던거 알고리즘별로 2문제씩 쭉쭉 풀어나가면서 다시 한회차 돌리기

24/01/28:
	구현, 시뮬레이션 / 완전탐색 / 그리디 / DP / 이진탐색, 투포인터 2회차 완료

24/01/30:
	BFS / DFS 완료 목표
	2트 이렇게 돌리고, 3트까지 완료하고 복기문제 리스트 만들어놓기
	실력을 빠르게 다시 돌려놓는 복기문제 !
```
...
진수 변환:
    ...
    3460 이진수 P
    8741 이진수 합 P
        시간 및 메모리 제약 빡셈(=공식 및 함수 활용 해야함)
        sum_num = num*(num+1)//2, bin(sum_num[2:])
...
백준 9655 돌 게임 PT P
백준 1463 1로 만들기 T PT
백준 11726 2xN 타일링 P PT
백준 2579 계단 오르기 P T
    1차원 DP 
백준 12865 평범한 배낭 - 0/1 knapsack PT T
    2차원 dp (무게, 가치)
...
백준 30792 랭킹 - 이진탐색 / bisect_left P
백준 1920 수 찾기 - 이진탐색 / bisect_left, right P P
백준 2805 나무 자르기 T PT
    이코테 떡볶이 떡 만들기 - 이진탐색 / bisect 풀이 쓰지 못함
백준 10816 숫자카드2 P P
    이코테 정렬된 배열에서 특정 수의 개수 구하기 - bisect_left, right



백준 2559 수열 - 투포인터 PT P
백준 3273 두수의합 - 투포인터 PT P
    양쪽에서 포인터가 출발하는 문제
백준 2470 두용액 - 투포인터 PT T
    양쪽에서 포인터가 출발하는 문제
백준 1806 부분합 - 투포인터 PT T
    왼쪽에서 포인터 출발 (=구간합)
백준 1644 소수의 연속합 - 투포인터 T PT
...
DFS 실버
백준-11724 연결 요소의 개수 P P
    BFS - 이코테 음료수 얼려먹기(격자) 의 그래프 버젼
백준-11403 경로 찾기 PT PT
    문제 너무 좋음
    BFS - 방향그래프 의 탐색
백준-2667 단지번호붙이기
